...
class Player(pg.sprite.Sprite):
    ...
    def load_animations(self):
        ...
        self.moving_animation_left = [pg.transform.flip(image, True, False)
    ...
    def handle_attack_mode(self):
        if self.attack_mode and pg.time.get_ticks() - self.timer > self.attack_interval:
            self.attack_mode = False
            self.timer = pg.time.get_ticks()
            self.fireball_timer = pg.time.get_ticks()

    def handle_animation(self):
        if not self.charge_mode and self.charge_power > 0:
            self.attack_mode = True

        if self.animation_mode and not self.attack_mode and pg.time.get_ticks() - self.timer > self.interval:
...
class Enemy(pg.sprite.Sprite):
    def __init__(self, folder):
        ...
        self.hp = 200
        self.healthbar_edge = pg.Rect(20, 20, 200, 20)
...
class Game:
    def __init__(self, mode, wizards):
        ...
        pg.display.set_caption("Корейская битва магов")
    ...
    def draw(self):
        ...
        pg.draw.rect(self.screen, "black", (20, 20, 200, 20), 2)
        pg.draw.rect(self.screen, "green", (22, 22, self.player.hp - 4, 15))
        pg.draw.rect(self.screen, "black", (SCREEN_WIDTH - 220, 20, 200, 20), 2)
        pg.draw.rect(self.screen, "green", (SCREEN_WIDTH - 218, 22, self.enemy.hp - 4, 15))
